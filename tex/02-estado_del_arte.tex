...
Android, Inc. fue fundada en Palo Alto, California en Octubre del 2003 por Andy Rubin, Rich Miner, Nick Sears y Chris White. Su objetivo era desarrollar dispositivos móviles más inteligentes, con un mayor foco en la localización del dueño y en su personalización.\\
...
\begin{itemize}
\item \textbf{Kernel Linux:} La capa más profunda es su núcleo Linux, un sistema operativo abierto, portable y seguro. Para cada pieza de hardware, como la cámara o el bluetooth, existe un driver dentro del kernel, que permite a la capa superior hacer uso de ella, por lo que funciona como una capa de abstracción. El kernel además se encarga de la gestión de los diversos recursos del dispositivo, como la energía o la memoria, elementos de comunicación, procesos, etc.

\item \textbf{Bibliotecas:} La segunda capa en el stack contiene bibliotecas nativas, las cuales están escritas en C o C++, y son compiladas para la arquitectura específica del dispositivo. En la mayoría de los casos el fabricantes es quien se encarga de instalarla en su dispositivo. Las bibliotecas incluidas en esta capa son: motor gráfico OpenGL, sistema de gestión de base de datos SQLite, cifrado de comunicaciones SSL, motor de manejo de tipos de letra FreeType, entre otras.
...
\item \textbf{Administrador de recursos (Resource Manager):} A través de este administrador se podrá acceder a los elementos que no forman parte del código, como imágenes, sonidos, layouts, etc. 

\item \textbf{Administrador de ubicaciones (Location Manager):} Permite obtener la posición geográfica actual del dispositivo a través de GPS o redes.

\item \textbf{Administrador de sensores (Sensor Manager):} Permite la manipulación de distintos sensores del dispositivo, como el acelerómetro, giroscopio, brújula, sensor de proximidad, etc.
...
Al comenzar a desarrollar una aplicación de Android, generalmente se crea un proyecto usando un IDE (Integrated Development Environment) como Eclipse o IntelliJ. El proyecto contendrá código fuente en Java y recursos, que pueden consistir en imágenes, sonidos, videos, archivos XML, entre otros. 


\begin{figure}[h!]
\centering
      \includegraphics[width=16cm]{Imagenes/proceso_compilacion}
\caption{Proceso de compilación de aplicaciones Android. Fuente: Elaboración Propia basada en \cite{10}}
\label{fig:Fig69}
\end{figure}

Cuando se genera un proyecto \cite[p.~14]{10} lo que ocurre es que se generan archivos .class en base al código fuente Java. Luego se compilan estos archivos .class generándose archivos ejecutables Dalvik (archivos .dex), los cuales pueden ser ejectuados por la máquina virtual Dalvik que está disponible en todos los dispositivos Android.\\

Tal como se puede ver en la figura \ref{fig:Fig69}, al compilar un proyecto se colocan los archivos .dex y el resto de los archivos del proyecto en uno llamado Android Package (archivo .apk). Este contiene todos los archivos necesarios para ejecutar la aplicación, incluyendo los .dex, recursos compilados, recursos sin compilar, y una versión binaria del Android Manifest.\\

El \textit{Android Manifest} es un archivo que especifica información esencial, que el sistema debe tener antes de ejecutar la aplicación. Además, por razones de seguridad todas las aplicaciones de Android deben ser firmadas digitalmente con un certificado.\\

Finalmente, el ADB (Android Debug Bridge) permite que el IDE se comunique con un dispositivo físico de Android o un emulador para poder ejectuar la aplicación.
...
Al comenzar el desarrollo de una aplicación Android, se debe decidir cuál va a ser la API mínima a la que se dará soporte. Esto significa que el desarrollador deberá usar métodos mayoritariamente de esa API de Android, ya que si desea usar métodos de una API superior, debe específicar que esa porción de código se ejecutará sólo para dispositivos que tengan esa versión de la API de Android o una superior, y no para el resto, lo cual dificulta la legibilidad del código. Esto también tendrá repercusiones al momento de que un usuario desee instalar la aplicación, ya que si su dispositivo cuenta con una versión como Froyo o Eclair, lo más probable es que no pueda instalar prácticamente ninguna de las aplicaciones disponibles en Google Play, la tienda en que se encuentran todas las aplicaciones que suben los desarrolladores.\\

Android actualiza mes a mes las estadísticas relativas al número de dispositivos que tiene cada versión del sistema operativo \cite{2}. Esto ayuda a tener una guía sobre cuál va a ser la API mínima soportada. En la figura \ref{fig:Fig4} se muestran las estadísticas correspondientes al mes de Abril del 2014. Esta información es recolectada durante los últimos 7 días de cada mes. Además se ignoran las versiones que tienen menos de un 0.1\%. Se puede apreciar que la versión que hoy en día es dominante, corresponde a Jelly Bean con más de un 60\%. La versión más reciente del sistema operativo, KitKat, tiene sólo un 5.3\% debido principalmente a que los operadores y fabricantes aún no tienen listas sus versiones personalizadas de KitKat, en las que pueden incluir nuevas funcionalidades o quitar lo que estimen conveniente.\\
\begin{figure}[h!]
\centering
...
Cuando ocurre esto, el comportamiento por defecto es destruir y volver a iniciar la \textit{Activity}. Se deberían tener en cuenta las siguientes preguntas:
\begin{itemize}
\item ¿Se dibuja de forma correcta todos los elementos en la pantalla?

\item ¿La aplicación mantiene el estado original? La \textit{Activity} no debería perder nada que el usuario haya ingresado en la UI.
\end{itemize}

\paragraph{Cambios de configuración}
También pueden ocurrir otros cambios más generales en el sistema, como un cambio de idioma. Este tipo de cambios desencadena el comportamiento por defecto de destruir y volver a iniciar la \textit{Activity}. 

\paragraph{Factores externos}
...
Si la aplicación está inactiva y ya ha pasado a segundo plano, lo más probable es que el sistema destruya la o las \textit{Activities} de la aplicación para liberar recursos como memoria. Es por ello que es necesario testear el ciclo de vida de la \textit{Activity} para corroborar si se destruye y se vuelva a comenzar de forma exitosa, sin pérdida del estado anterior.

Para el caso de testing de clases en aplicaciones Android, es posible que ocurran dos casos: que las clases realicen llamadas a la API de Android o que sólamente usen bibliotecas nativas de Java.

\paragraph{Testing de clases en Java \cite{19}}
\par Si las clases que se tienen no hacen llamadas a la API de Android, se puede usar el framework de JUnit \cite{49} sin ninguna restricción.
...
\par Si se quieren hacer tests que usen la API de Android, éstos necesitan llevarse a cabo en un dispositivo con Android. Esto hace que la ejecución de los tests tome más tiempo, principalmente porque la biblioteca \textit{android.jar} no contiene el código del framework de Android. Este archivo es únicamente usado al momento de compilar una aplicación. Una vez que la aplicación está instalada, se utilizará la biblioteca \textit{android.jar} que está en el dispositivo.
...
Como ya se mencionó anteriormente, existen muchas versiones del sistema operativo Android vigentes hoy en día. Esto priva al desarrollador de métodos útiles al momento de programar su aplicación, ya que se debe establecer una API mínima. En base a las estadísticas que provee Android, la mayoría de los desarrolladores decide dar soporte desde Gingerbread en adelante. Si por ejemplo, el desarrollador desea usar como API mínima Ice Cream Sandwich y quiere usar métodos de Jelly Bean, deberá especificar en el código fuente que esa parte sólo tiene que ser ejecutada si el dispositivo del usuario es mayor o igual a la API 16. La siguiente porción de código \cite{11} es un ejemplo de lo que los desarrolladores deben hacer: \\
\begin{lstlisting}
if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
    // Aquí va código superior o igual a la API 16
}
\end{lstlisting}
...
Existen cuatro tamaños generales de pantalla \cite{2}: pequeña (small), normal, grande (large) y extra grande (xlarge). Para optimizar la experiencia del usuario, muchas veces se debe implementar una interfaz distinta para un tipo específico de pantalla. Esto se lleva a cabo a través de un archivo escrito en XML que define los elementos presentes en la interfaz gráfica.\\

Si se desea usar el mismo archivo para todas las pantallas simplemente se deben ir guardando estos archivos en el directorio de recursos del proyecto, \textit{res/layout}. En caso que se quiera agregar un archivo distinto para un tipo de pantalla, además de tener un archivo en la ruta antes mencionada, se debe crear una nueva carpeta, añadiendo el sufijo que corresponda a cada tamaño de pantalla. \\
...
\label{fig:Fig6}
\end{figure}
Por ejemplo, si se quisiera agregar una interfaz distinta para pantallas más grandes (large), se debe crear el directorio \textit{res/layout-large} y añadir el archivo que define la interfaz, por lo que existirán dos versiones de este archivo, uno en el directorio antes señalado, mientras que el otro estará en el directorio general, \textit{res/layout}, que será para todo el resto de las pantallas.\\
...
Android categoriza las resoluciones de cada pantalla en base a la densidad de pixeles que poseen. Esta medida es la cantidad de píxeles que hay por pulgada. Existen cinco tipos de densidades \cite{2}: baja (ldpi), media (mdpi), alta (hdpi), extra-alta (xhdpi) y extra-extra-alta (xxhdpi). Además existe otro tipo de densidad, la cual es usada principalmente para televisores (tvdpi).\\

Muchas veces es necesario agregar diferentes versiones del mismo recurso gráfico. Por ejemplo, si sólo se agrega un recurso gráfico en la resolución mdpi, este probablemente se verá bien en resoluciones bajas como ldpi y mdpi, pero para las más altas, como xxhdpi o xhdpi, la imagen será redimensionada y se perderá mucha calidad.\\
...
Además de los tamaños de pantalla y sus resoluciones, existen otras características muy importantes que los desarrolladores deben tener en cuenta. Una de las principales es la RAM con la que cuenta el dispositivo, ya que es allí donde se cargan las instrucciones que ejecuta el procesador. También es muy importante el procesador y la cantidad de núcleos con los que cuenta. Aunque la mayoría de los dispositivos tiene cámara, también se debe tener en cuenta que algunos no la tienen. Por último, muchas veces los fabricantes como Samsung, hacen cambios en el sistema operativo, cambiando cosas como por ejemplo, la interfaz del sistema, lo cual produce diferentes experiencias en cada dispositivo, y muchas veces genera errores que el desarrollador no se espera.
...
Google se dió cuenta de que esto se estaba convirtiendo en un problema, ya que faltaban herramientas que permitieran a los desarrolladores llevar sus versiones betas a los usuarios antes de que se publiquen de forma oficial, por lo que el año 2013 presentaron en la Conferencia Google I/O \cite{37} una actualización en su sistema de subida de versiones para dar soporte a versiones beta, aunque aún le falta mayor interacción con los usuarios.
