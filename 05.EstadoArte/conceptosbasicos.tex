...
En este capitulo se realizará un recorrido por los conceptos claves que ayudarán a tener una mayor comprensión del problema de motion planning. 
...
Cuando no se controlan todos los grados de libertad se habla entonces de un robot no holonómico.

Esto es lo que sucede con los automóviles, ya que si bien uno puede moverse hacia adelante y hacia atrás, como también efectuar cambios de dirección gracias al volante, los movimiento laterales no están permitidos. La figura ~\ref{fig:Fig6} muestra los movimientos que debe efectuar un vehículo para estacionarse.
...
En la mayoría de los casos existen dos alternativas para modelar la geometría de los elementos que se encuentran dentro de un entorno: una representación del contorno de los elementos o una representación sólida. Si por ejemplo se modela la geometría de una naranja a través de la representación por contorno se buscaría una ecuación similar a la de una esfera. Si se empleara en cambio la representación sólida se debería describir todo el conjunto de punto que están dentro de la naranja.

Para comenzar se debe definir que se trabajará en un entorno $\mathcal{W}$, el cual puede ser un mundo 2D en el que $\mathcal{W} = \mathbb{R}^2$ o un mundo en 3D en el que $\mathcal{W} = \mathbb{R}^3$. Para casos complejos se pueden usar espacios dimensionales más altos.
...
A continuación se presentarán dos métodos que permiten construir representaciones de obstáculos y de robots usando un conjunto de primitivas, como también de funciones polinomiales \cite{2}.

\subsection{Modelos Poligonales}
Cada región de obstáculos se representa tomando una combinación finita de primitivas. Además, se usa teoría de conjuntos, esto implica que $\mathcal{O}$ quedará definida por un número finito de uniones, intersecciones entre primitivas. Cada primitiva será definida como $H_i$.

Primero se trabajará con polígonos convexos en un entorno 2D, por lo que se define $\mathcal{W} = \mathbb{R}^2$. Para que un polígono sea convexo debe cumplir con lo siguiente, si se traza una recta entre cualquier par de puntos al interior del polígono, esta permanece dentro y no se intersecta con ninguna arista.
...
Un obstáculo $\mathcal{O}$ con forma poligonal convexa de m lados  puede expresarse como
\begin{center}
$\mathcal{O} = H_1\cap H_2\cap H_3\cap ...\cap H_m$
\end{center}
Ahora si se considera un obstáculo que no es convexo, éste puede ser expresado como la unión de varios obstáculos convexos, tal como se define a continuación:
\begin{center}
$\mathcal{O} = \mathcal{O}_1\cup \mathcal{O}_2\cup \mathcal{O}_3\cup ...\cup \mathcal{O}_n$
\end{center}
A través de esta definción se pueden representar obstáculos mucho más complejos. Aunque hay muchas formas de descomponer un obstáculo en componentes convexas se debe ser cuidadoso al realizar este procedimiento, buscando optimizar la selección para mejorar el desempeño de los algoritmos futuros a utilizar. Lo ideal es que se utilice una cantidad mínima de primitivas.

Todo lo anterior puede ser fácilmente extrapolado a un entorno $\mathcal{W} = \mathbb{R}^3$.

\subsection{Modelos Semi-Algebraicos}
En los modelos poligonales que se vieron en la sección anterior, se usaron primitivas las cuales eran funciones lineales. Para este tipo de modelos pueden usarse funciones de cualquier grado polinomial con coeficientes reales y variables $x$ e $y$.

Sea el caso de un entorno 2D. Una representación sólida puede ser definida de la forma
\begin{center}
...
Las técnicas vistas anteriormente sirven para definir tanto el área de los obstáculos como la del robot. Sea $\mathcal{O}$ como un área de obstáculos, subconjunto de $\mathcal{W}$, y  a $\mathcal{M}$ como el robot,. En el problema básico de motion planning los obstáculos son estáticos, pero se requiere mover el robot $\mathcal{M}$, por lo que hay que introducir algunas transformaciones que permitan el desplazamiento de $\mathcal{M}$ desde un punto de origen a uno destino.
...
Un robot rígido $\mathcal{M} \subset \mathbb{R}^2$ se traslada usando dos parámetros, $x_t, y_t \in \mathbb{R}$. Usando la función de transformación vista en la sección anterior se definirá $h$ como
\begin{center}
$h(x,y) = (x+x_t,y+y_t)$
\end{center}
Una representación del contorno de $\mathcal{M}$ puede ser trasladada transformando cada uno de los vértices del polígono. Por lo que cada punto $(x_i,y_i)$ de un polígono con $i$ vértices es reemplazado por $(x_i+x_t,y_i+y_t)$.
...
\end{center}
Esto se puede apreciar en la figura ~\ref{fig:Fig25}, en donde se aplica la transformación de traslación sobre un polígono de 5 vértices.
\begin{figure}[h!]
\centering
	    \includegraphics[width=6cm]{Imagenes/25EstadoArte}
\caption{Traslación en un polígono de 5 vértices}
\label{fig:Fig25}
\end{figure}
...
Se usará una matriz de rotación para explicar la transformación,
 \begin{center}
 $R(\theta) =
 \begin{pmatrix}
  \cos\theta & -\sin\theta \\
...
En varias ocasiones el robot realizará una rotación y posteriormente se desplazará, por lo que es necesario una función de transformación que permita combinar la rotación y la traslación. Además hay que tener presente que llevar a cabo un movimiento de rotación y posteriormente uno de traslación es distinto a que si se realizaran al revés, por lo que no son conmutativos.
...
Las transformaciones vistas anteriormente para 2D son conceptualmente similares a lo que se debe realizar en un entorno 3D.
...
En este capítulo se han introducido una serie de conceptos básicos para entender el problema de motion planning. Entre ellos se encuentran la holonomicidad de un robot, relacionada con el grado de libertad que posee un robot. Además, se debe tener en cuenta la representación del robot dentro de un espacio 2D o 3D, contando con dos opciones tradicionales: Modelos Poligonales y Modelos Semi-Algebraicos. Los tipos de movimientos que tiene a su disposición un robot son igual de importantes, estas transformaciones son: Traslación, Rotación y ambos combinados. Finalmente se introdujo el concepto clave de espacio de configuración. Este ayuda a especificar la posición de cada punto del robot en el entorno, redefiniendo el espacio de trabajo y ayudando a que el robot no colisione con los obstáculos presentes.

En el próximo capítulo se revisarán los distintos avances que han existido en esta área, desde la década de los 70 hasta la época actual. Finalmente se presentan algunas variantes que existen del problema original. 